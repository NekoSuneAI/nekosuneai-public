...
let userAgent = `NekoSuneAIBOT/${packageJson.version} ${config.VRCACC
  .username}`;
...
function VRCFriends() {
  ...
  function HandleFriendOffline(data) {
    if (config.addons.vrcapi.toggles.blscan) {
      fetch(`https://nekologger.nekosunevr.co.uk/v5/games/api/vrchat/yoinker/check/${data.userId}`)
        ...
        .then(blcheck => {
          if (blcheck.blacklisted == true) {
            ...
            console.log(
              `${data.user
                .displayName} been Global Blacklised by NekoSune Community`
            );
  ...
  function AcceptFriendRequest(data) {
    ...
    if (config.addons.vrcapi.toggles.blfr) {
      fetch(`https://nekologger.nekosunevr.co.uk/v5/games/api/vrchat/yoinker/check/${data.senderUserId}`)
        .then(res11 => res11.json())
        .then(blcheck => {
          if (blcheck.blacklisted == true) {
            ...
            NotificationsApi.acceptFriendRequest(data.id)
              .then(async () => {
                await sleep(3000);
                fetch(`http://localhost:9065/v4/self/get`)
                  ...
                  .then(async resp => {
                    ...
                    oscClient.send(
                      new Message(
                        "/chatbox/input",
                        `Thank You for Friend Request ${data.senderUsername}, now i have over ${resp
...
async function GetWorldInstance(req) {
  return new Promise((resolve, reject) => {
    WorldsApi.getWorldInstance(req.body.world, req.body.instance)
      .then(resp => {
        const response = {
          status: 200,
          data: resp.data
...
async function GetWorld(req) {
  return new Promise((resolve, reject) => {
    WorldsApi.getWorld(req.body.world)
      .then(resp => {
...
async function GetUser(req) {
  return new Promise((resolve, reject) => {
    UsersApi.getUser(req.body.userid)
      .then(resp => {
        const response = {
          status: 200,
          data: resp.data
