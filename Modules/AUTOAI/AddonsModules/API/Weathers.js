...
async function WeatherGrabber(originalText) {
    ...
    try {
        ...
        weather.getAllWeather(function(err, weatherData) {
            if (weatherData.weather[0].description == null || weatherData.weather[0].description == undefined) {
                writeToLogFile(`[Weather API] Error: Not found ${processText(originalText, commandPatterns)}`);
                resolve(`We cant find that Data called: ${processText(originalText, commandPatterns)}, Try again or Please Say it Again.`);
            } else {
                var datafound = `Weather in ${processText(originalText, commandPatterns)}: ${weatherData.weather[0].description}, tempture is ${weatherData.main.temp}, Humidity is: ${weatherData.main.humidity} with Wind: ${weatherData.wind.speed} MPH`;
                resolve(datafound);
            }
        })
    } catch (error) {
