...
function findCountryByName(countryName) {
    ...
    const foundCountry = timezone.find(country =>
        country.countryName.toLowerCase() === lowerCasecountryName
    );
...
async function TimezonesGrabber(originalText) {
    try {
        ...
        const foundCountry = findCountryByName(processText(originalText, commandPatterns));

        // Get the current time in the specified region
        const selectedTime = new Date().toLocaleString('en-US', { timeZone: foundCountry.timeZone });
...
//////////////// OLD DATA //////////////////////
function formatTimeWithAMPMFromDateString(dateString) {
