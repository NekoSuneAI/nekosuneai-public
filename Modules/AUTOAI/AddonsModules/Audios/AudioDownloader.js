...
async function DownloadFile(source, mp3Url, filepath, filename) {
    ...
    if (source == "memes") {
        ...
        speakText(`This Invalid Command for Audio, Please say or stop with song name or meme number`);
    }
}

function writeAudioFile(arrayBuffer, outputPath) {
    const fs = require('fs');
    ...
    writeToLogFileMusic(`Audio file written to ${outputPath}`);
}
